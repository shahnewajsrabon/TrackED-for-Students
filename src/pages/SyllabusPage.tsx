...
import { BookOpen, Plus, Check, Circle, Trash2, ChevronDown, Clock } from 'lucide-react';
...
export default function SyllabusPage() {
  ...
  return (
    ...
    >
      <div className="bg-brand-surface p-8 md:p-10 rounded-[2.5rem] border border-brand-border shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-8">
        <div>
        ...
        {subjects.length > 0 && (
          <div className="relative min-w-[240px]">
            <select 
              className="w-full appearance-none bg-brand-bg border border-brand-border rounded-2xl px-5 py-4 font-bold text-brand-text-primary focus:outline-none focus:ring-2 focus:ring-primary shadow-sm text-lg cursor-pointer transition-colors hover:border-gray-300"
      ...
      {subjects.length === 0 ? (
        <div className="text-center py-24 bg-brand-surface rounded-[2.5rem] border border-brand-border border-dashed shadow-sm">
        ...
        <div className="grid md:grid-cols-3 gap-10">
          <div className="md:col-span-2 space-y-8">
            <div className="bg-brand-surface rounded-[2.5rem] border border-brand-border shadow-sm p-8 md:p-10">
              <div className="flex justify-between items-end mb-8">
                 <div>
                   <SubjectBadge name={activeSubject.name} color={activeSubject.color} />
                   <h2 className="text-3xl font-black mt-4 text-brand-text-primary">Chapter Topics</h2>
                 </div>
              ...
              <div className="flex gap-4 mb-8 bg-brand-bg p-5 rounded-[2rem] border border-brand-border shadow-sm">
                <input 
                  type="text" 
                  ...
                  className="flex-1 bg-white border border-brand-border px-5 py-3 rounded-2xl focus:outline-none focus:ring-2 focus:ring-primary shadow-sm text-lg font-medium transition-colors"
                />
                <motion.button 
                  ...
                  className="bg-primary hover:bg-primary/90 disabled:opacity-50 text-white px-6 rounded-2xl font-bold shadow-md transition-all flex items-center justify-center"
                >
              ...
              <div className="space-y-3">
                <AnimatePresence>
                  {(activeSubject.syllabus || []).map(topic => (
                    <motion.div 
                      ...
                      className={`group flex items-center justify-between p-5 rounded-3xl border transition-all shadow-sm mb-3 ${
                    ...
                    >
                      <div className="flex items-center gap-5 flex-1 cursor-pointer" onClick={() => handleToggleTopic(topic.id, topic.status)}>
                        <button 
                          className={`min-w-8 w-8 h-8 rounded-full border-2 flex items-center justify-center transition-all ${
                        ...
                        <span className={`font-bold text-lg transition-colors ${topic.status === 'completed' ? 'line-through text-brand-text-secondary opacity-60' : 'text-brand-text-primary'}`}>
                          {topic.title}
                        </span>
                      </div>
                      <button 
                        onClick={() => handleDeleteTopic(topic.id)}
                        className="text-gray-400 hover:text-danger opacity-0 group-hover:opacity-100 transition-opacity p-3 bg-white hover:bg-danger/10 border border-brand-border rounded-xl shadow-sm"
                      >
                ...
                {(!activeSubject.syllabus || activeSubject.syllabus.length === 0) && (
                  <div className="text-center py-16 text-brand-text-secondary font-bold text-lg bg-brand-bg rounded-[2rem] border-2 border-dashed border-brand-border">
          ...
          <div className="space-y-8">
            <div className="bg-brand-surface rounded-[2.5rem] border border-brand-border shadow-sm p-8 max-h-[80vh] overflow-y-auto scrollbar-hide">
                ...
                {subjects.map(s => {
                  ...
                  return (
                    <button 
                      ...
                      className={`w-full text-left p-5 rounded-3xl border transition-all ${s.id === activeSubject.id ? 'bg-brand-bg border-primary shadow-md transform scale-[1.02]' : 'bg-brand-bg border-brand-border hover:border-gray-300 hover:shadow-sm'}`}
                    >
